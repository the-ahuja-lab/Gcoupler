...
def compund_synthesis(path='',CavID='',ligand_count=500,detected='F'):
        CavID=str(CavID)
        if path.endswith('/'):
                pass
        else:
...
def cavity_detect(path='',res_list='',ligand_count=500):
        ...
        if os.path.exists(LigB_path+pdb_nm):
                ...
                if list(res_percent.values()).count(max(res_percent.values())) > 1:
                        ...
                        return compund_synthesis(path,list(Dscore.keys())[list(Dscore.values()).index(max(Dscore.values()))],ligand_count=ligand_count,detected='T')
                else:
                        ...
                        f.close()
                        os.system('echo Cav%:'+str(max(res_percent.values())))
                        return compund_synthesis(path,CAVITY,ligand_count=ligand_count,detected='T')
