...
def synthetic_interaction(pdbqt='',method='KS-test',p_val=0.05,plot='Density'):
    res,cwdir = bepred(pdbqt)
    cutoffs=[]
...
def synthetic_decoys(cf=-7,decoy_csv=''):
    if os.path.exists('Progress.st'):
        with open('Progress.st')as fin:
            for line in fin:
                if 'PDBPath' in line:
    ...
    if len(decoy_csv)>0:
        ...
        if len(lb.count('HAB'))<250 or len(lb.count('Decoy'))<150:
